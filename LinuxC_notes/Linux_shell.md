...
###### 文件系统类型包括：
- vfat:Windows长文件系统
- ntfs:Win NT、XP、Vista及Win 7中广泛使用的高级文件系统
...
###### 查看磁盘的详细使用情况 df
常用参数  -h : 默认情况下单位为字节，该参数可将单位按大单位取整显示
...
- - -
##### 后台模式
	使进程在后台执行，格式为: (命令;命令;命令...)&
    此时可使用jobs命令查看后台任务

##### 协程
	在后台生成一个子shell,在子shell中执行命令
    coproc命令用于启动协程，格式为: coproc 命令;命令;命令....
	该命令会启动一个后台任务
    
**注:生成子shell的速度慢，成本高，尽量避免嵌套子shell**

- - -

#####  bash shell的命令分为两种
- 外部命令，也叫文件系统命令，通常位于/bin, /user/bin, /sbin或/user/sbin中，执行外部命令会创建出一个包含全新环境的子进程
- 内建命令， 顾名思义，存在于shell内，执行速度更快效率更高

**部分命令有外部和内建等多种实现,可用type查看命令类型**
